...
Documents: `GET /documents?folder_id=&q=&trashed=true&sort=created|updated|name&order=asc|desc`, `POST /documents`, `POST /documents:import` (multipart -- creates the document and its first version), `GET /documents/{id}`, `PATCH /documents/{id}`, `PUT /documents/{id}/version-counter`, `DELETE /documents/{id}`, `POST /documents/{id}:restore`.
