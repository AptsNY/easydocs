...
public class EasyDocsDbContext(DbContextOptions<EasyDocsDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        // Content search (issue #12): the tsvector is generated by Postgres from Text — 'simple'
        // config on purpose, because documents come in any language and stemming one of them
        // mis-stems the rest. GIN is the standard tsvector index.
        b.Entity<DocumentText>(e =>
        {
            e.HasKey(x => x.DocumentId);
            e.HasOne<Document>().WithOne().HasForeignKey<DocumentText>(x => x.DocumentId).OnDelete(R);
            e.Property(x => x.SearchVector)
                .HasComputedColumnSql("to_tsvector('simple', \"Text\")", stored: true);
